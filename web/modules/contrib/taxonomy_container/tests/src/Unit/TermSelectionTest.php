 ...
 *
 * @todo Turn this into Kernel test.
 */
class TermSelectionTest extends UnitTestCase {
  ...
  public function setUp() {
    ...
    $this->entityRepository->getTranslationFromContext(Argument::type(TermInterface::class))->will(
      function ($args) {
        return $args[0];
      }
    );
  ...
  public function testGetReferenceableEntities(array $configuration, array $vocabularies, array $expected_result) {
    ...
    $this->entityTypeBundleInfo->getBundleInfo('taxonomy_term')->willReturn($bundle_info);
  ...
  protected function getMockTerm($id, $label, $parent = 0) {
    ...
    $term->expects($this->any())
      ...
      ->will($this->returnValueMap([
        ['parent', (object) ['target_id' => $parent]],
        ['depth', $parent === 0 ? 0 : 1],
      ]));
    return $term;
  }
